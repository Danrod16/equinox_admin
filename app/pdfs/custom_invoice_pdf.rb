class CustomInvoicePdf
  ...
  def content
    ...
    bounding_box([65, 600], width: 200, height: 20) do
      text "EQ0#{@custom_invoice.sequence}-#{@custom_invoice.created_at.strftime('%Y')}", align: :center, valign: :center, character_spacing: 1, color: "FFFFFF"
      stroke_bounds
    end
    ...
    move_down 50
    text "Cliente:  #{@custom_invoice.client}", leading: 2
    text "Dirección:  #{@custom_invoice.address}", leading: 2
    text "NIF:  #{@custom_invoice.nif}", leading: 2
